define(function(require, exports, module) {
    ...
    console.table(data);
    console.log('%cAn Alexander Gugel production', 'color: #0066cc;');
    ...
    var Element = UIElement.extend({
        ...
        goHelix: function() {
            ...
            this.setPosition(x, y, z, { duration: 1000, curve: 'inOutQuad' });
        },
        ...
        goParaflow: function() {
            ...
            this.setRotation(Math.random()*Math.PI*10, Math.random()*Math.PI*10, Math.random()*Math.PI*10, { duration: duration, curve: 'linear' });

            function _genRandomLoc() {
                return (window.innerWidth/2 - Math.random()*window.innerWidth)*10;
            }

            this.setPosition(_genRandomLoc(), _genRandomLoc(), _genRandomLoc(), { duration: duration, curve: 'linear' }, function() {
                this.paraflowing && this.goParaflow();
            }.bind(this));
        },

        // make each Element glow in opacity
        glow: function() {
            this.setOpacity(0.5 + 0.5*Math.random(), {
                duration: 1000
            }, this.glow.bind(this));
        }
    });
    ...
    var periodicTable = window.periodicTable = new (UIContainer.extend({
        constructor: function() {
            ...
            // Development: Used for checking the origin
        ...
        goParaflow: function() {
            for (var i = 0; i < this.elements.length; i++)
                this.elements[i].setDelay(i*25).goParaflow();
        },
    ...
    // rotate on drag
    ux.on('dragUpdate', function(event) {
    ...
    ux.on('scrollUpdate', function(event) {
        periodicTable._transitionableTransform.translate.halt();
    ...
    goHomeButton.on('click', periodicTable.goHome.bind(periodicTable));
    ...
    goHelixButton.on('click', periodicTable.goHelix.bind(periodicTable));
    ...
    goSphereButton.on('click', periodicTable.goSphere.bind(periodicTable));
    ...
    goGridButton.on('click', periodicTable.goGrid.bind(periodicTable));
    ...
    goParaflowButton.on('click', periodicTable.goParaflow.bind(periodicTable));
    ...
    app.addChild(ux);
    app.addChild(periodicTable);
    app.addChild(goHelixButton);
    app.addChild(goHomeButton);
    app.addChild(goSphereButton);
    app.addChild(goGridButton);
    app.addChild(goParaflowButton);
});
